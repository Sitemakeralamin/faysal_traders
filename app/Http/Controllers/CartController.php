...
class CartController extends Controller
{
    ...
    public function add_cart(Request $request)
    {
        ...
        if(!is_null($product)) {
            ...
            if($add_stock_status == 1) {
                if($stock_qty >= $qty) {
                    $price = optional($stock_info)->price??0;
                    $old_price = 0;
                    $new_price = $price;
                    $discount = optional($product)->discount_amount??0;
                    $discount_type = $product->discount_type??'non';
                    if($discount_type <> 'no') {
                                ...
                                if(!is_null($product_info)) {
                                    ...
                                    $cart_sidebar .= '<div class="minicart__product--items d-flex">
                                            <div class="minicart__thumb">
                                                <a href="'.route('product.show', $product_info->slug).'"><img style="height: 135px !important;" class="shadow rounded p-1" src="'.asset('images/product/' .optional($product_info)->thumbnail_image).'" alt="prduct-img"></a>
                                            </div>
                                            <div class="minicart__text">
                                                <h3 class="minicart__subtitle h4"><a href="'.route('product.show', $product_info->slug).'">'.$product_info->title.'</a></h3>
                                                ...
                                                <div class="minicart__text--footer d-flex align-items-center justify-content-between">
                                                    ...
                                                    '.csrf_field().'
                                                        ...
                                                        <button type="submit" class="minicart__product--remove"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-delete"><path d="M21 4H8l-7 8 7 8h13a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2z"></path><line x1="18" y1="9" x2="12" y2="15"></line><line x1="12" y1="9" x2="18" y2="15"></line></svg></button>
                                                    </form>
                                                    </div>
                                                </div>
                                            </div>
    ...
    public function remove_cart(Request $request)
    {
        ...
        Session::forget('coupon_discount');
        if(count($carts)>0){
            return back()->with('success', 'Item removed successfull');
        }else{
            return redirect()->route('index')->with('error', 'Item removed successfull');
        }
    }
