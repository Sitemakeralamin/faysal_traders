...
class SliderSideBannerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->image){
            if (File::exists('images/slider-right-side-banner/'.$sliderSideBanner->image)){
                File::delete('images/slider-right-side-banner/'.$sliderSideBanner->image);
            }
            $image = $request->file('image');
            $img = time() . '.' . $image->getClientOriginalExtension();
            $location = public_path('images/slider-right-side-banner/'. $img);
