                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <form action="{{ route('wishlist.remove', $wishlist->id) }}" method="POST">
                                    @csrf
                                    <a @if($wishlist->product->type == 'single') onclick="addToCart({{ $wishlist->product->id }})" @else {{--onclick="quick_view({{ $wishlist->product->id }})"--}} href="{{ route('product.show', $wishlist->product->slug) }}" @endif class="continue__shipping--btn primary__btn border-radius-5 rounded-pill" style="color: #fff;">Add To Cart</a>
